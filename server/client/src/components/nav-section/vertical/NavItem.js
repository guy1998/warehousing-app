...
export function NavItemSub({ item, open = false, active = false, onOpen }) {
  ...
  const { stockAlertProducts } = useSelector((state) => state.product);

  const renderContent = (
    <>
      <DotIcon active={active} />
      <ListItemText disableTypography primary={title} />
      {title.toLowerCase() === 'stock alert' && stockAlertProducts.length > 0 && <StockAlertIcon />}
      {children && <ArrowIcon open={open} />}
    </>
  );
...
export function StockAlertIcon() {
  return <WarningRounded sx={{ color: 'orange' }} />;
}
