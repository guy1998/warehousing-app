...
import { getStockAlertProducts } from '../../redux/slices/product';
// routes
...
export default function EcommerceProductList() {
  ...
  const { stockAlertProducts, isLoading } = useSelector((state) => state.product);
  ...
  useEffect(() => {
    dispatch(getStockAlertProducts());
  }, [dispatch]);

  useEffect(() => {
    if (stockAlertProducts.length) {
      setTableData(stockAlertProducts);
    }
  }, [stockAlertProducts]);
  ...
  return (
    <Page title="Stock Alert Product List">
      <Container maxWidth={false}>
        <HeaderBreadcrumbs
          heading="Stock Alert Product List"
          links={[
