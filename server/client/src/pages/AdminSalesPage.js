...
import { getAllSales } from '../sections/@dashboard/sales/scripts/sales-scripts';
...
function AdminSalesPage() {
  ...
  const handleView = (soldProd, selectedType) => {
  ...
  useEffect(() => {
    getAllSales(notification, navigate, myFilter).then((data) => {
                  ...
                  productsDisplayed.map((row, index) => (
                    <TableRow key={row.productName} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      ...
                      <TableCell align="left">{row.quantity}</TableCell>
                        ...
                        {(row.priceHoreca && !row.soldPrice)&& <TableCell align="left">${type === 'horeca' ? row.priceHoreca?.$numberDecimal : row.priceRetail?.$numberDecimal}</TableCell>}
                    </TableRow>
                  ))}
              </TableBody>
            </Table>
