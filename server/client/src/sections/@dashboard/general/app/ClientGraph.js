...
export default function ClientGraph() {
  ...
  useEffect(async () => {
    ...
    await fetch(`${process.env.REACT_APP_BACKEND_URL}/client/zones/retrieve`, requestOptions)
      .then((response) => response.text())
      .then((result) => {
        const resultJSON = JSON.parse(result);
        setZones(resultJSON.map((item) => item.zone));
        setCount(resultJSON.map((item) => item.count));
